...
from tf_utils import load_dataset, load_dataset_cat, random_mini_batches, convert_to_one_hot, predict
...
trainList_load = glob.glob(r"D:\neuralNet_data\Cat_data_labelled\train\train\*.jpg")
testList_load = glob.glob(r"D:\neuralNet_data\Cat_data_labelled\test1\test1\*.jpg")

#cat 0-12497
#dog 0-12499 (12498-24997)


cat_start = 0
dog_start = 12498
cat_end = cat_start + 5000
dog_end = dog_start + 5000

trainList = np.concatenate((trainList_load[cat_start:cat_end],trainList_load[dog_start:dog_end]))
testList = np.concatenate((trainList_load[cat_end+1:cat_end+51],trainList_load[dog_end+1:dog_end+51]))
...
X, Y = create_placeholders(12288, num_labels)
print ("X = " + str(X))
print ("Y = " + str(Y))
...
def initialize_parameters():
    ...
    W3 = tf.get_variable("W3", [num_labels,12], initializer = tf.contrib.layers.xavier_initializer(seed = 1))
    b3 = tf.get_variable("b3", [num_labels,1], initializer = tf.zeros_initializer())
    ### END CODE HERE ###
...
fname = r"D:\neuralNet_data\Cat_data_labelled\train\train\dog.5815.jpg"
image = np.array(ndimage.imread(fname, flatten=False))
