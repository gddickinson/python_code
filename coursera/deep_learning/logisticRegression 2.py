# -*- coding: utf-8 -*-
"""
Created on Mon Sep  4 12:35:32 2017
...
train_set_x_orig = np.array([np.array(processImage(fname, num_px)) for fname in trainList])
train_set_y = np.array([np.array(label(fname)) for fname in trainList])
train_set_y = train_set_y.reshape(1,train_set_y.shape[0])
test_set_x_orig = np.array([np.array(processImage(fname, num_px)) for fname in testList])
test_set_y = np.array([np.array(label(fname)) for fname in testList])
...
def propagate(w, b, X, Y):
    ...
    ### START CODE HERE ### (≈ 2 lines of code)
    dw = np.array(1/m * np.dot(X,np.transpose((A-Y))))
    db = 1/m * np.sum(A-Y)
    ### END CODE HERE ###
