...
for fileName in fileList:
    ...
    #create colour masks  
    green_mask = file.imageview.getProcessedImage()[:,:,1] > 250
    blue_mask = file.imageview.getProcessedImage()[:,:,2] > 20
    notBlue_mask = np.logical_or(file.imageview.getProcessedImage()[:,:,2] < 20, green_mask)
    blue_mask = notBlue_mask == False
    
    red_mask = file.imageview.getProcessedImage()[:,:,0] > 20
    red_mask = np.logical_and(red_mask,notBlue_mask)
    
    
    #show mask
    #Window(blue_mask, 'blue_mask') 
    #Window(notBlue_mask, 'notBlue_mask') 
    #Window(red_mask, 'red_mask')
    #3333Window(green_mask, 'green_mask')
    ...
    #only proceed if 6 points detected
    
    if len(allCenteroids) != 6:
        continue
    ...
    #create colour masks
    green_mask_rotated = rotatedImg[:,:,1] > 250
    blue_mask_rotated = rotatedImg[:,:,2] > 20
    notBlue_mask_rotated = np.logical_or(rotatedImg[:,:,2] < 20, green_mask_rotated)
    blue_mask_rotated = notBlue_mask_rotated == False
    
    red_mask_rotated = rotatedImg[:,:,0] > 20
