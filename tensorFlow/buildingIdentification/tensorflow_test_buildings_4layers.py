...
from tf_utils import load_dataset, load_dataset_cat, random_mini_batches, convert_to_one_hot, predict_building_4layers, linear_function, sigmoid, cost, one_hot_matrix, ones, create_placeholders
import glob
import scipy
...
def initialize_parameters():
    ...
    W4 = tf.get_variable("W4", [num_labels,12], initializer = tf.contrib.layers.xavier_initializer(seed = 1))
    b4 = tf.get_variable("b4", [num_labels,1], initializer = tf.zeros_initializer())
...
print ("CPU run time  = " + endCPU_Time)
...
my_image_prediction = predict_building_4layers(my_image, parameters, imageVectorSize)
...
filename_pickle = r"C:\\Google Drive\\code\\python_code\\tensorFlow\\buildingIdentification\\parameters\\parameter_" +time.strftime("%Y%m%d-%H%M%S")
