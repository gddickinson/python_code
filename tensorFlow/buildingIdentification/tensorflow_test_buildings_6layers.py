...
from tf_utils import load_dataset, load_dataset_cat, random_mini_batches, convert_to_one_hot, predict_building_6layers, linear_function, sigmoid, cost, one_hot_matrix, ones, create_placeholders
import glob
import scipy
...
#%matplotlib inline
#np.random.seed(1)
#with random.seed(1) at epoch 3600 cost increases and does not decrease
...
def initialize_parameters():
    ...
    W1 = tf.get_variable("W1", [30,imageVectorSize], initializer = tf.contrib.layers.xavier_initializer(seed = 1))
    ...
    W4 = tf.get_variable("W4", [12,25], initializer = tf.contrib.layers.xavier_initializer(seed = 1))
    b4 = tf.get_variable("b4", [12,1], initializer = tf.zeros_initializer()) 
    W5 = tf.get_variable("W5", [12,12], initializer = tf.contrib.layers.xavier_initializer(seed = 1))
    b5 = tf.get_variable("b5", [12,1], initializer = tf.zeros_initializer())       
    W6 = tf.get_variable("W6", [num_labels,12], initializer = tf.contrib.layers.xavier_initializer(seed = 1))
    b6 = tf.get_variable("b6", [num_labels,1], initializer = tf.zeros_initializer())
...
def model(X_train, Y_train, X_test, Y_test, learning_rate = 0.0001,
          num_epochs = 4500, minibatch_size = 32, print_cost = True):
    """
...
my_image_prediction = predict_building_6layers(my_image, parameters, imageVectorSize)
...
filename_pickle = r"C:\\Google Drive\\code\\python_code\\tensorFlow\\buildingIdentification\\parameters\\parameter_" +time.strftime("%Y%m%d-%H%M%S") + "_6layer"
