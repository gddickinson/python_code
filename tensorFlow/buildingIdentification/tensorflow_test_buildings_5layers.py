...
from tf_utils import load_dataset, load_dataset_cat, random_mini_batches, convert_to_one_hot, predict_building_5layers, linear_function, sigmoid, cost, one_hot_matrix, ones, create_placeholders
import glob
import scipy
...
def initialize_parameters():
    ...
    W4 = tf.get_variable("W4", [12,12], initializer = tf.contrib.layers.xavier_initializer(seed = 1))
    b4 = tf.get_variable("b4", [12,1], initializer = tf.zeros_initializer())        
    W5 = tf.get_variable("W5", [num_labels,12], initializer = tf.contrib.layers.xavier_initializer(seed = 1))
    b5 = tf.get_variable("b5", [num_labels,1], initializer = tf.zeros_initializer())
...
startCPU_Time = time.clock()    
parameters = model(X_train, Y_train, X_test, Y_test)
endCPU_Time = time.clock() - startCPU_Time
print ("CPU run time  = " + str(endCPU_Time))
...
my_image_prediction = predict_building_5layers(my_image, parameters, imageVectorSize)
...
filename_pickle = r"C:\\Google Drive\\code\\python_code\\tensorFlow\\buildingIdentification\\parameters\\parameter_" +time.strftime("%Y%m%d-%H%M%S") + "_5layer"
