...
class WeightedEdge(Edge):
    def __init__(self, src, dest, totalDist = 0, outdoorDist = 0):
        self.src = src
        self.dest = dest
    ...
    def __str__(self):
        return '{0}->{1}'.format(self.src, self.dest) +' ' + str((self.totalDist,self.outdoorDist))
...
class WeightedDigraph(Digraph):
    ...
    def addEdge(self, weightedEdge):
        ...
        totalDist = float(weightedEdge.getTotalDistance())
        outdoorDist = float(weightedEdge.getOutdoorDistance())
        if not(src in self.nodes and dest in self.nodes):
            raise ValueError('Node not in graph')
