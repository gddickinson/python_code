...
class NewsStory(object):
    def __init__(self, guID, newsTitle, newsSubject, newsSummary, link):
        self.guID = guID
        self.newsTitle = newsTitle
        self.newsSubject = newsSubject
...
class WordTrigger(Trigger):
    ...
    def isWordIn(self, text):    
        ...
        for symbol in string.punctuation:
            testText = testText.replace(symbol, ' ')
        splitText = []
        splitText = testText.split()
        if self.word in splitText:
            return True
        return False
...
class NotTrigger(Trigger):
    ...
    def evaluate (self,story):
        if self.A_trigger.evaluate(story) == True:
            return False
        else:
            return True
...
class AndTrigger(Trigger):
    ...
    def evaluate (self,story):
        if self.A_trigger.evaluate(story) == True and self.B_trigger.evaluate(story) == True:
            return True
        else:
            return False    
...
class OrTrigger(Trigger):
    def __init__(self, A_trigger, B_trigger):
        ...
        self.B_trigger = B_trigger
    def evaluate (self,story):
        if self.A_trigger.evaluate(story) == True or self.B_trigger.evaluate(story) == True:
            return True
        else:
            return False      
...
def makeTrigger(triggerMap, triggerType, params, name):
    ...
    if triggerType == 'TITLE':
        triggerMap[name] = TitleTrigger(test)
    
    if triggerType == 'SUBJECT':
        triggerMap[name] = SubjectTrigger(test) 
    
    if triggerType == 'SUMMARY':
        triggerMap[name] = SummaryTrigger(test) 
    
    if triggerType == 'PHRASE':
        triggerMap[name] = PhraseTrigger(test)
...
def main_thread(master):
    ...
    try:
        ...
        triggerlist = readTriggerConfig("/home/george/Documents/stanford_cs106a/ProblemSet7/triggers.txt")
