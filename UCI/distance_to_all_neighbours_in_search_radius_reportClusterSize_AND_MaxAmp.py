...
def distances(filename1, filename2, output):
    ...
    comparisonSet = np.vstack((x2,y2, z2))    
    ...
    def getNeighbours(x, y, comparisonSet, searchRadius):
        ...
        for i in range (comparisonSet[0].size):
            if abs(x-comparisonSet[0][i]) < searchRadius and abs(y-comparisonSet[1][i]) < searchRadius:
                ...
                keptZ.append(comparisonSet[2][i])
        answer = np.vstack((keptX,keptY,keptZ))
    ...
    def getDistances(x,y, searchSet, searchRadius):
        ...
        for i in range (searchSet[0].size):
            ...
            if dist < searchRadius:
                ...
                clusterDensity.append(searchSet[2][i])
        #print(distance)
        return distance, clusterDensity
    ...
    def allDists(dataSet,comparisonSet,searchRadius):
        ...
        for i in range (dataSet[0].size):
            distance1, clusterDensity1 = (getDistances(dataSet[0][i],dataSet[1][i],getNeighbours(dataSet[0][i],dataSet[1][i],comparisonSet,searchRadius),searchRadius))
            ...
            for s in range (len(distance1)):
                ...
                maxAmp.append(dataSet[2][i])
                clusterDensity.append(clusterDensity1[s])
            print(float((i/dataSet[0].size)*100),"%");
        return dataDist, maxAmp, clusterDensity   
...
#==============================================================================
    ...
    distanceSet, maxAmpSet, clusterDensitySet = allDists(data,comparisonSet,searchRadius)
    #distanceSet, maxAmpSet = allDists(data,comparisonSet,searchRadius)  
    #distanceSet = allDistsNoRedundancies(data,comparisonSet,searchRadius)

    outputSet = np.vstack((distanceSet, maxAmpSet, clusterDensitySet))
    #outputSet = np.vstack((distanceSet, maxAmpSet))
    np.savetxt(output, np.transpose(outputSet), delimiter=',')
...
pathBase = r'J:\WORK_IN_PROGRESS\Puff Anchoring\Nocodozole\SY5Y_Nocodazole_NEW_Analysis\Nocodazole\puff_XY_data\\'
 
path1 = pathBase + r'\data1_XY\\'
path2 = pathBase + r'\data1_XY\\'
path3 = pathBase + r'\results\\'
...
for FileName in files:
    ...
    output = path3 + '_result_allDistances_' + FileName   
