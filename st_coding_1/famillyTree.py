...
class Family(object):
    ...
    def cousin(self, a, b):
        ...
        if self.is_child(a,b):
            return (-1,0);
        
        if self.is_child(b,a):
            return (-1,0);
        
        def nodes_to_common_ancestor(a,b):
            iteration = 1           
            cousin_nodeA = self.names_to_nodes[a]
            cousin_nodeB = self.names_to_nodes[b]
            
            if cousin_nodeA == self.root:
                return iteration
                
            if cousin_nodeB == self.root:
                return iteration
        
            if (cousin_nodeA.get_parent()) == (cousin_nodeB.get_parent()):
                return iteration

            else:
                if (cousin_nodeA.get_parent()) == self.root:
                    #print self.root
                    return iteration
                
                elif (cousin_nodeB.get_parent()) == self.root:
                    #print self.root
                    return iteration                    
                else:    
                    iteration += nodes_to_common_ancestor(str((cousin_nodeA.get_parent())),str((cousin_nodeB.get_parent())))
                               
                return iteration
        
        cousin_type = (nodes_to_common_ancestor(a,b))-1        

            


        def distance_to_root(node):
            nodeName = self.names_to_nodes[node]
            iteration = 1            
            if nodeName == self.root:
                return iteration
            else: iteration += distance_to_root(str(nodeName.get_parent()))
            
            return iteration
                
        #print distance_to_root(a)
        #print distance_to_root(b)

        degree_removed = distance_to_root(a)-distance_to_root(b)
        if degree_removed < 0:
            degree_removed = degree_removed *-1
        
        ans = (cousin_type,degree_removed);
        return ans
