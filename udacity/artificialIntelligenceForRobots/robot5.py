# -*- coding: utf-8 -*-
"""
Created on Wed Nov  9 17:35:18 2016
...
class robot:
    ...
    def move(self, motion): 
        ...
        #add noise
        steeringAngle = random.gauss(steeringAngle, self.steering_noise)
        distance = random.gauss(distance, self.distance_noise)

        if steeringAngle > 0.001:
            turningAngle = (distance/self.length)*tan(steeringAngle)
            turnRadius = distance/turningAngle
            cx = self.x - sin(result.orientation)*turnRadius
            cy = self.y + cos(result.orientation)*turnRadius       
            xPrime = cx + sin(result.orientation + turningAngle)* turnRadius
            yPrime = cy - cos(result.orientation + turningAngle)* turnRadius
            orientationPrime = (result.orientation + turningAngle) % (2*pi)
        
        else: #straight
            xPrime = self.x + distance * cos(result.orientation)
            yPrime = self.y + distance * sin(result.orientation)
            orientationPrime = (result.orientation + steeringAngle) % (2*pi)

        result.x = xPrime
        result.y = yPrime
        result.orientation = orientationPrime
    ...
    def sense(self, add_noise = 1): 
        ...
        for i in range(len(landmarks)):
            ...
            bearing %= (2.0 * pi) #normalize between 0 and 2pi
            Z.append(bearing)
        return Z 
