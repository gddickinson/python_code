...
def online_slam(data, N, num_landmarks, motion_noise, measurement_noise):
    ...
    for k in range(len(data)):
        ...
        # expand the information matrix and vector by one position

        nlist = [0,1] + list(range(4, dim+2))
        Omega = Omega.expand(dim+2, dim+2, nlist, nlist)
        Xi = Xi.expand(dim+2, 1, nlist, [0])
